...
public class SEDMailBoxTest extends TestUtils {
    ...
    public static void startClass()
        ...
        try {
            ...
            Files.copy(SEDMailBoxTest.class.getResourceAsStream(
                    "/pmode-conf.xml"), Paths.get(
                            SED_HOME + "/pmode-conf.xml"),
                    StandardCopyOption.REPLACE_EXISTING);
            System.getProperties().put(SEDSystemProperties.SYS_PROP_HOME_DIR,
                    SED_HOME);
            ...
            memfFactory = Persistence.createEntityManagerFactory(
                    PERSISTENCE_UNIT_NAME);
            ...
            mTestInstance.mutUTransaction = new MockUserTransaction(
                    mTestInstance.memEManager.getTransaction());
    ...
    private void assertModifyOutMail(OutMail om,
            SEDOutboxMailStatus startOMStatus, ModifOutActionCode oac,
            SEDOutboxMailStatus endOMStatus,
        ...
        try {
            ...
            assertNotNull("Response/RControl/@returnValue",
                    mer.getRControl().getReturnValue());
            assertEquals("Response/RControl/@returnValue",
                    mer.getRControl().getReturnValue().intValue(),
                    SVEVReturnValue.OK.getValue());
            assertNotNull("Response/RData", mer.getRData());
            assertNotNull("Response/RData/OutEvent",
                    mer.getRData().getOutEvent());
            assertEquals("Mail id", om.getId(),
                    mer.getRData().getOutEvent().getMailId());
            assertEquals("Sender box", om.getSenderEBox(),
                    mer.getRData().getOutEvent().getSenderEBox());

            OutMail omtest = getOutMail(om.getId());
            assertNotNull("OutMail not exists in db", omtest);
            assertEquals("Wrong modified end status", endOMStatus.getValue(),
                    omtest.getStatus());

        } catch (SEDException_Exception ex) {
            assertNotNull("Error " + ex.getMessage() +
                    " is not exptected to be thrown", ecExpected);
            assertNotNull("SEDException_Exception/FaultInfo", ex.getFaultInfo());
            assertNotNull("SEDException_Exception/FaultInfo/ErrorCode",
                    ex.getFaultInfo().getErrorCode());
            assertEquals("Erro code", ecExpected,
                    ex.getFaultInfo().getErrorCode());
        }
    ...
    private void assertThrowErrorOnSubmit(SubmitMailRequest smr,
            String assertMessage, SEDExceptionCode ecExpected) {
    ...
    private void storeInMail(InMail mail)
            throws StorageException, HashException {
        ...
        try {
            ...
            if (mail.getInPayload() != null &&
                    !mail.getInPayload().getInParts().isEmpty()) {
                for (InPart p : mail.getInPayload().getInParts()) {
                    ...
                    if (p.getValue() != null) {
                        ...
                        if (fIn.exists()) {
                            fout = msuStorageUtils.storeOutFile(p.getMimeType(),
                                    fIn);
                        }
                    }
                    ...
                    if (fout != null) {
                        ...
                        if (Utils.isEmptyString(p.getName())) {
                            p.setName(p.getFilename().substring(
    ...
    private void storeUpdateOutMail(OutMail mail, SEDOutboxMailStatus st)
            throws StorageException, HashException {
        if (mail.getId() != null) {
            EntityManager me = null;
            try {
                ...
                if (mail.getOutPayload() != null &&
                    ...
                    for (OutPart p : mail.getOutPayload().getOutParts()) {
                        ...
                        if (p.getValue() != null) {
                            ...
                            File fIn = new File(p.getFilepath());
                            if (fIn.exists()) {
                                fout = msuStorageUtils.storeOutFile(
                                        p.getMimeType(), fIn);
                            }
                        }
                        // set MD5 and relative path;
    ...
    public void test_A_SubmitMail()
        ...
        assertNotNull("Response/RControl/@returnValue",
                mr.getRControl().getReturnValue());
        assertEquals("Response/RControl/@returnValue",
                mr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.
                getValue());
        assertNotNull("Response/RData", mr.getRData());
        assertNotNull("Response/RData/@mailId", mr.getRData().getMailId());
        assertNotNull("Response/RData/@submitDate",
                mr.getRData().getSubmittedDate());
        assertNotNull("SenderMessageId", mr.getRData().getSenderMessageId());
        assertEquals("SenderMessageId", mr.getRData().getSenderMessageId(),
                smr.getData().getOutMail().
                getSenderMessageId());
     ...
     */
    @Test
    public void test_B_SubmitMail_ValidationOfRequest()
            throws Exception {
        ...
        smr.setData(null);
        assertThrowErrorOnSubmit(smr, "Missing Data",
                SEDExceptionCode.MISSING_DATA);
        smr.setData(dt);
        // check Data/OutMail
     ...
     */
    @Test
    public void test_C_SubmitMail_ExistsMail()
            throws Exception {
        ...
        assertEquals("Second submission Response/RControl/@returnValue",
                secmr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.WARNING.getValue());
        assertEquals("Second submission Response/RData/@submitDate",
                mr.getRData().getSubmittedDate(), secmr.getRData().
                getSubmittedDate());
        assertEquals("Second submission Response/RData/@mailId",
                mr.getRData().getMailId(), secmr.getRData().getMailId());
        assertEquals("Second submission Response/RData/@senderMessageId",
                mr.getRData().getSenderMessageId(), secmr.
                getRData().getSenderMessageId());
     ...
     */
    @Test
    public void test_D_GetOutMailList()
            throws Exception {
        ...
        assertNotNull("Response/RControl/@returnValue",
                mlr.getRControl().getReturnValue());
        assertEquals("Response/RControl/@returnValue",
                mlr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.getValue());
        assertNotNull("Response/RControl/@resultSize",
                mlr.getRControl().getResultSize());
        assertNotNull("Response/RControl/@responseSize",
                mlr.getRControl().getResponseSize());
        assertNotNull("Response/RControl/@StartIndex",
                mlr.getRControl().getStartIndex());
        assertNotNull("Response/RData", mlr.getRData());
        assertEquals("Response/RControl/@resultSize -> getOutMails().size()",
                mlr.getRData().getOutMails().size(), mlr.
                getRControl().getResultSize().intValue());
        assertEquals("Response/RControl/@resultSize -> getOutMails().size()",
                mlr.getRData().getOutMails().size(), mlr.
                getRControl().getResponseSize().intValue());
        ...
        assertEquals("Response/RControl/@resultSize -> getOutMails().size()",
                mlr.getRData().getOutMails().size(), mlr.
                getRControl().getResultSize().intValue());
        assertEquals("Response/RControl/@resultSize -> increased", TEST_CNT +
                iResCNt, mlr.getRData().getOutMails().
                size());
        ...
        assertEquals("Service Response/RControl/@returnValue",
                mlr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.getValue());
        assertEquals("Test service parameter", 1,
                mlr.getRData().getOutMails().size());

        assertEquals("Test service response id", om.getId(),
                mlr.getRData().getOutMails().get(0).getId());

        //test search for action
        omr.getData().setAction((om.getAction()));
        mlr = mTestInstance.getOutMailList(omr);
        omr.getData().setAction(null);
        assertEquals("Action Response/RControl/@returnValue",
                mlr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.getValue());
        assertEquals("Test Action parameter", 1,
                mlr.getRData().getOutMails().size());
        assertEquals("Test  Action response id", om.getId(),
                mlr.getRData().getOutMails().get(0).getId());
        ...
        assertEquals("ConversationId Response/RControl/@returnValue",
                mlr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.getValue());
        assertEquals("Test ConversationId parameter", 1,
                mlr.getRData().getOutMails().size());
        assertEquals("Test ConversationId response id", om.getId(),
                mlr.getRData().getOutMails().get(0).getId());
        ...
        assertEquals("SenderEBox Response/RControl/@returnValue",
                mlr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.getValue());
        assertEquals("Test SenderEBox parameter", 1,
                mlr.getRData().getOutMails().size());
        assertEquals("Test SenderEBox response id", om.getId(),
                mlr.getRData().getOutMails().get(0).getId());
        ...
        assertEquals("ReceiverEBox Response/RControl/@returnValue",
                mlr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.getValue());
        assertEquals("Test ReceiverEBox parameter", 1,
                mlr.getRData().getOutMails().size());
        assertEquals("Test ReceiverEBox response id", om.getId(),
                mlr.getRData().getOutMails().get(0).getId());

        //test search for SubmittedDateFrom
        omr.getData().setSubmittedDateFrom(dt);
        mlr = mTestInstance.getOutMailList(omr);
        assertEquals("SubmittedDateFrom Response/RControl/@returnValue",
                mlr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.getValue());
        assertEquals("Test SubmittedDateFrom parameter", TEST_CNT + 1,
                mlr.getRData().getOutMails().size());
        ...
        assertEquals("SubmittedDateFrom Response/RControl/@returnValue",
                mlr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.getValue());
        assertEquals("Test SubmittedDateFrom parameter", TEST_CNT,
                mlr.getRData().getOutMails().size());
        ...
        assertEquals("SubmittedDateFrom Response/RControl/@returnValue",
                mlr.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.getValue());
        assertEquals("service RControl/@ResultSize", TEST_CNT + iResCNt + 1,
                mlr.getRControl().getResultSize().
                intValue());
        assertEquals("service RControl/@getResponseSize", TEST_CNT,
                mlr.getRControl().getResponseSize().intValue());
    }
     ...
     */
    @Test
    public void test_E_GetOutMailEventList()
            throws Exception {
        ...
        assertEquals("Response/RControl/@returnValue",
                ...
                SVEVReturnValue.OK.getValue());
        assertNotNull("Response/RControl/@resultSize",
                mer.getRControl().getResultSize());
        assertNotNull("Response/RControl/@responseSize",
                mer.getRControl().getResponseSize());
        assertNotNull("Response/RControl/@StartIndex",
                mer.getRControl().getStartIndex());
        assertNotNull("Response/RData", mer.getRData());
        assertEquals("Response/RControl/@resultSize -> OutMailEvent().size()",
                mer.getRData().getOutEvents().size(),
                mer.getRControl().getResultSize().intValue());
        assertEquals("Response/RControl/@resultSize -> OutMailEvent().size()",
                mer.getRData().getOutEvents().size(),
                mer.getRControl().getResponseSize().intValue());
        assertTrue("Response/RControl/@resultSize -> OutMailEvent().size()",
                mer.getRData().getOutEvents().size() > 0);
        ...
        assertEquals("Response/RControl/@returnValue",
                ...
                SVEVReturnValue.OK.getValue());
        assertTrue("Response/RControl/@resultSize -> OutMailEvent().size()",
                mer.getRData().getOutEvents().size() == iResult);
    }
     ...
     */
    @Test
    public void test_F_GetInMailList()
            throws Exception {
        ...
        assertNotNull("Response/RData", mlr.getRData());
        assertEquals("Response/RControl/@resultSize -> getOutMails().size()",
                mlr.getRData().getInMails().size(), mlr.
                getRControl().getResultSize().intValue());
        assertEquals("Response/RControl/@resultSize -> getOutMails().size()",
                mlr.getRData().getInMails().size(), mlr.
                getRControl().getResponseSize().intValue());
        assertTrue("Response/RControl/@resultSize -> OutMailEvent().size()",
                mlr.getRData().getInMails().size() > 0);
        ...
        assertEquals("Response/RControl/@resultSize -> getOutMails().size()",
                ...
                getRControl().getResultSize().intValue());
        assertEquals("Response/RControl/@resultSize -> increased", TEST_CNT +
                iResCNt, mlr.getRData().getInMails().
                size());
        ...
        assertEquals("Test service response id", im2.getId(),
                mlr.getRData().getInMails().get(0).getId());
        ...
        assertEquals("Test Action parameter", 1,
                mlr.getRData().getInMails().size());
        assertEquals("Test  Action response id", im2.getId(),
                mlr.getRData().getInMails().get(0).getId());
        ...
        assertEquals("Test ConversationId parameter", 1,
                mlr.getRData().getInMails().size());
        assertEquals("Test ConversationId response id", im2.getId(),
                mlr.getRData().getInMails().get(0).getId());
        ...
        assertEquals("Test SenderEBox parameter", 1,
                mlr.getRData().getInMails().size());
        assertEquals("Test SenderEBox response id", im2.getId(),
                mlr.getRData().getInMails().get(0).getId());
        ...
        assertEquals("Test ReceiverEBox parameter", 1,
                mlr.getRData().getInMails().size());
        assertEquals("Test ReceiverEBox response id", im2.getId(),
                mlr.getRData().getInMails().get(0).getId());
     ...
     */
    @Test
    public void test_G_GetInMailEventList()
            throws Exception {
        ...
        assertNotNull("Response/RData", mer.getRData());
        assertEquals("Response/RControl/@resultSize -> OutMailEvent().size()",
                mer.getRData().getInEvents().size(), mer.
                getRControl().getResultSize().intValue());
        assertEquals("Response/RControl/@resultSize -> OutMailEvent().size()",
                mer.getRData().getInEvents().size(), mer.
                getRControl().getResponseSize().intValue());
        assertTrue("Response/RControl/@resultSize -> OutMailEvent().size()",
                mer.getRData().getInEvents().size() > 0);
    ...
    public void test_H_GetInMail()
        ...
        assertArrayEquals("Response/RData/getPyloadPart",
                im.getInPayload().getInParts().get(0).getValue(), mer.
                getRData().getInMail().getInPayload().getInParts().get(0).getValue());
     ...
     */
    @Test
    public void test_I_ModifyInMail()
            throws Exception {
        ...
        assertEquals("Mail id", im.getId(),
                mer.getRData().getInEvent().getMailId());
        assertEquals("Receiver box", im.getReceiverEBox(),
                mer.getRData().getInEvent().getReceiverEBox());
        ...
        assertEquals("Response/RControl/@returnValue",
                mrs.getRControl().getReturnValue().intValue(),
                SVEVReturnValue.OK.getValue());
        assertEquals("Status", SEDInboxMailStatus.DELIVERED.getValue(),
                mrs.getRData().getInMail().getStatus());
        assertEquals("Date", mer.getRData().getInEvent().getDate(),
                mrs.getRData().getInMail().getStatusDate());
