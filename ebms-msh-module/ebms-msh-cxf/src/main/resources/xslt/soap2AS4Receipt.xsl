...
xmlns:sed="http://msh.sed.si">
	...
	<xsl:template match="eb3:UserMessage">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="/S12:Envelope/S12:Header/eb3:Messaging">
						<xsl:apply-templates select="//eb3:UserMessage"/>
					</xsl:when>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="eb3:UserMessage">
			<!--TODO without eveloping as4receipt fails on xsd validation -->
			<sed:MessageData>
			<xsl:copy-of select="current()"/>
			</sed:MessageData>
