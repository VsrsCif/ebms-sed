...
public class EBMSUtils {
  ...
  public UserMessage createUserMessage(PMode pm, MSHOutMail mo, String senderDomain, Date timestamp)
    ...
    MessageInfo mi =
        createMessageInfo(mo.getMessageId(), senderDomain, mo.getRefToMessageId(), timestamp);
    ...
    piTo.setValue(mo.getReceiverName() == null || mo.getReceiverName().isEmpty() ? mo
        .getReceiverEBox() : mo.getReceiverName());
...
/*
  ...
  /*
   * public static File createSoapLogFile(MSHMail mm, boolean isRequest, boolean isOutgoingMail)
   * throws IOException { File f; String prefix = EbMSConstants.File_PREFIX_SOAP_Message + (mm !=
   * null ? mm.getConversationId() + "-" + mm.getAction() : "no_conv_id") + "-"; f =
   * File.createTempFile(prefix, isRequest ? EbMSConstants.File_Suffix_SOAP_Message_Request :
   * EbMSConstants.File_Suffix_SOAP_Message_Response, isOutgoingMail ?
   * Settings.getInstance().getOutboxFolder() : Settings.getInstance().getInboxFolder());
   * 
   * return f; }
   */
}
