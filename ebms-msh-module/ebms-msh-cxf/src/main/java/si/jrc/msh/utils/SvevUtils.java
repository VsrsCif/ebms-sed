...
public class SvevUtils {
  ...
  public void vaildateMail(MSHOutMail mail) throws MSHException {
     ...
     * mail.getMailType().equals(MSHOutOutboxMailType.DeliveryAdvice) && (mail.getRefToMessageId()
     * == null || mail.getRefToMessageId().trim().isEmpty()) ){ throw new
     * MSHException(MSHExceptionCode.MissingDataInMail, "DeliveryAdvice must have RefToMessageId!");
     * }
     */
    if (!merrLst.isEmpty()) {
