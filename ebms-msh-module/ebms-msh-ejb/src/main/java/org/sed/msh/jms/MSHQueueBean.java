...
public class MSHQueueBean implements MessageListener {
  ...
  public void onMessage(Message msg) {
    ...
    try {
      ...
      LOG.logError(t, "Bad message with no property:'" + SEDValues.EBMS_QUEUE_PARAM_MAIL_ID + "'!",
          ex);
      return;
    }
    ...
    try {
      ...
      LOG.logError(t,
          "Bad message with no property:'" + SEDValues.EBMS_QUEUE_PARAM_PMODE_ID + "'!", ex);
      return;
    }
    ...
    try {
      ...
      String errDesc =
          "Error reading pmodes for id: '" + pModeID + "'. Err:" + pex.getMessage()
              + ".  Message with id '" + idMsg + "' is not procesed!";
    ...
    if (pMode == null) {
      String errDesc =
          "PMode with id: '" + pModeID + "' not exists! Message with id '" + idMsg
              + "' is not procesed!";
      LOG.logError(t, errDesc, null);
      try {
        mDB.setStatusToOutMail(mail, SEDOutboxMailStatus.ERROR, errDesc);
    ...
    try {
      ...
      LOG.logError(t, "Error occurred while submitting mail  to receiver MSH:'" + mail.getId()
          + "'!", ex);
      ...
      if (pMode.getReceptionAwareness() != null && pMode.getReceptionAwareness().getRetry() != null) {
        ...
        if (iRet < rty.getMaxRetries()) {
          ...
          try {
            try {
              mDB.setStatusToOutMail(mail, SEDOutboxMailStatus.SCHEDULE, "Resend message in '"
                  + lDelay + "'ms");
            } catch (StorageException ex1) {
              LOG.logError(t, "Error occurred while setting status SCHEDULE to MSHOutMail :'"
                  + mail.getId() + "'!", ex1);
              return;
            }
            ...
            try {
              ...
              LOG.logError(t,
                  "Error occurred while setting status ERROR to MSHOutMail :'" + mail.getId()
                      + "'!", ex2);
              return;
            }
