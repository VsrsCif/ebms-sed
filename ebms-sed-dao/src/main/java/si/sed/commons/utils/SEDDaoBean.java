...
public class SEDDaoBean implements SEDDaoInterface {
    ...
    protected <T> CriteriaQuery createSearchCriteria(Class<T> type,
            Object searchParams, boolean forCount,
        ...
        if (searchParams != null) {
            ...
            for (Method m : methodList) {
                ...
                if (Modifier.isPublic(m.getModifiers()) &&
                        m.getParameterCount() == 0 && !m.getReturnType().equals(
                                Void.TYPE) && (mName.startsWith("get") ||
                        mName.startsWith("is"))) {
                    String fieldName = mName.substring(mName.startsWith("get") ?
                            3 : 2);
                    ...
                    if (fieldName.endsWith("List") &&
                            searchValue instanceof List &&
                            !((List) searchValue).isEmpty()) {
                        lstPredicate.add(om.get(fieldName.substring(0,
                                fieldName.lastIndexOf("List"))).in(
     ...
     */
    @Override
    public <T> List<T> getDataList(Class<T> type, int startingAt,
            int maxResultCnt, String sortField, String sortOrder,
            Object filters) {
        long l = LOG.logStart(type, startingAt, maxResultCnt, sortField,
                sortOrder, filters);
        List<T> lstResult;
        try {
            CriteriaQuery<T> cq = createSearchCriteria(type, filters, false,
                    sortField, sortOrder);
        ...
        LOG.logEnd(l, type, startingAt, maxResultCnt, sortField, sortOrder,
                filters);
        return lstResult;
    }
    ...
    public <T> long getDataListCount(Class<T> type, Object filters) {
        ...
        CriteriaQuery<Long> cqCount = createSearchCriteria(type, filters, true,
                null, null);
    ...
    public List<MSHInMail> getInMailConvIdAndAction(String action, String convId) {
        ...
        Query q = memEManager.
                createNamedQuery(
                        "org.msh.ebms.inbox.mail.MSHInMail.getByConvIdAndAction",
                        MSHInMail.class);
    ...
    public SEDTaskExecution getLastSuccesfullTaskExecution(String type) {
        ...
        TypedQuery<SEDTaskExecution> tq = memEManager.createNamedQuery(
                "org.sed.ebms.cron.SEDTaskExecution.getByStatusAndType",
        ...
        try {
            ...
            LOG.logWarn(l, "No succesfull task execution for type: " + type,
                    null);
        }
    ...
    public <T> T getMailById(Class<T> type, BigInteger mailId) {
        ...
        TypedQuery<T> tq = memEManager.createNamedQuery(type.getName() +
                ".getById", type);
    ...
    public <T> List<T> getMailEventList(Class<T> type, BigInteger mailId) {
        long l = LOG.logStart(type, mailId);
        TypedQuery tq = memEManager.createNamedQuery(type.getName() +
                ".getMailEventList", type);
    ...
    public <T> boolean remove(T o) {
        ...
        try {
            mutUTransaction.begin();
            memEManager.remove(memEManager.contains(o) ? o :
                    memEManager.merge(o));
            mutUTransaction.commit();
            suc = true;
        } catch (NotSupportedException | SystemException | RollbackException |
     ...
     * @throws StorageException
     */
    public <T, E> void removeMail(Class<T> type, Class<E> typeEvent,
            BigInteger bi)
            throws StorageException {
        ...
        try {
            ...
            lst.stream().
                    forEach((e) -> {
                        memEManager.remove(memEManager.contains(e) ? e :
                                memEManager.merge(e));
                    });
            //remove mail
            memEManager.remove(memEManager.contains(mail) ? mail :
                ...
                IllegalStateException ex) {
            {
                ...
                String msg =
                        "Error occurred where removing mail type: '" + type +
                        "', id: '" + bi + "'! Err:" + ex.
                        getMessage();
    ...
    public void serializeInMail(MSHInMail mail, String applicationId)
        ...
        try {
                ...
                IllegalStateException ex) {
            {
                ...
                String msg = "Error occurred on serializing mail! Err:" +
                        ex.getMessage();
                LOG.logError(l, msg, ex);
                throw new StorageException(msg, ex);
            }
    ...
    public void setStatusToInMail(MSHInMail mail, SEDInboxMailStatus status,
        ...
        try {
            ...
            Query updq = memEManager.createNamedQuery(
                    SEDNamedQueries.UPDATE_INMAIL);
            ...
            if (iVal != 1) {
                ...
                String msg = "Status not setted to MSHInMail:" + mail.getId() +
                        " result: '" + iVal +
                ...
                IllegalStateException ex) {
            {
                ...
                String msg = "Error commiting status to incommingxmail: '" +
                        mail.getId() + "'! Err:" + ex.getMessage();
                LOG.logError(l, msg, ex);
                throw new StorageException(msg, ex);
            }
        }
    ...
    public void setStatusToOutMail(MSHOutMail mail, SEDOutboxMailStatus status,
        ...
        try {
            ...
            Query updq = memEManager.createNamedQuery(
                    SEDNamedQueries.UPDATE_OUTMAIL);
            updq.setParameter("id", mail.getId());
            updq.setParameter("statusDate", mail.getStatusDate());
            updq.setParameter("status", mail.getStatus());
            ...
            if (iVal != 1) {
                ...
                String msg = "Status not setted to MSHOutMail:" + mail.getId() +
                        " result: '" + iVal +
                        "'. Mail not exists or id duplicates?";
                LOG.logError(l, msg, null);
                throw new StorageException(msg, null);
                ...
                IllegalStateException ex) {
            {
                ...
                String msg = "Error commiting status to outboxmail: '" +
                        mail.getId() + "'! Err:" + ex.getMessage();
                LOG.logError(l, msg, ex);
                throw new StorageException(msg, ex);
            }
    ...
    public void updateInMail(MSHInMail mail, String statusDesc, String user)
        ...
        try {
                ...
                IllegalStateException ex) {
            {
                ...
                String msg = "Error occurred on update in mail: '" +
                        mail.getId() + "'! Err:" + ex.getMessage();
                LOG.logError(l, msg, ex);
                throw new StorageException(msg, ex);
            }
