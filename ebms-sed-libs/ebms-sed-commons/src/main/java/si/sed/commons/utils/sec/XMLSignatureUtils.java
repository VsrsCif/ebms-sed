...
public class XMLSignatureUtils {
  ...
  /**
     *
     */
  protected static final Logger mlgLogger = getLogger(XMLSignatureUtils.class.getName());
  ...
  private Document callTimestampService(String ireq, String wsldLocatin,
      String soapActionNamespace, String soapAction) throws SEDSecurityException {
    long t =
        logStart("SvevSignatureUtils.callTimestampService: params: req: '" + ireq + "' url: '"
            + wsldLocatin + "'");
    long tCall, tReceive;
    ...
    try {
      ...
      if (soapAction == null) {
        conn.setRequestProperty(HTTPHeader_SAOPAction, soapAction == null ? ""
            : (soapActionNamespace + soapAction));
      }
      ...
      mlgLogger.info("SvevSignatureUtils.callTimestampService: receive response in (" + tReceive
          + "ms)");
      ...
      throw new SEDSecurityException(CreateTimestampException, ex, ex.getMessage());
      ...
      if (conn != null) {
        ...
        mp.keySet().stream().forEach((s) -> {
          sw.append(s + " : " + mp.get(s));
        });
      }
      mlgLogger.error(sw.toString(), ex);
      throw new SEDSecurityException(CreateSignatureException, ex, ex.getMessage());
    } finally {
      if (conn != null) {
        try {
   ...
   */
  public List<Reference> createReferenceList(List<String[]> lst, XMLSignatureFactory fac)
      throws SEDSecurityException {
    long t = getTime();
    ...
    try {
      ...
      lst.stream().forEach((s) -> {
        lstRef.add(fac.newReference("#" + s[0], dm, null, s[1], getUUID(ID_PREFIX_Referece)));
      });
    } catch (NoSuchAlgorithmException ex) {
   ...
   * @throws SEDSecurityException
   */
  public SignedInfo createSignedInfo(List<String[]> lst, XMLSignatureFactory fac)
      throws SEDSecurityException {
    long t = getTime();
    SignedInfo si = null;
    try {
      ...
      si =
          fac.newSignedInfo(
              fac.newCanonicalizationMethod(INCLUSIVE, (C14NMethodParameterSpec) null),
              fac.newSignatureMethod(RSA_SHA1, null), lstRef, getUUID(ID_PREFIX_SignedInfo));
    } catch (NoSuchAlgorithmException ex) {
      logError("SvevSignatureUtils.createSignedInfo", ex.getMessage(), t, ex);
      throw new SEDSecurityException(NoSuchAlgorithm, ex, ex.getMessage());
    } catch (InvalidAlgorithmParameterException ex) {
   ...
   * @throws SEDSecurityException
   */
  public XMLStructure createXAdESQualifyingProperties(String sigId, String strSigValId,
      String strSigPropId, X509Certificate cert, Document doc) throws SEDSecurityException {
    long t = getTime();
  ...
  public Element getTimeStamp(String hash) throws SEDSecurityException {
    String reg = format(TIMESTAMP_REQUEST, hash, Calendar.getInstance().getTimeInMillis());
  ...
  public XMLSignatureFactory getXMLSignatureFactory() throws SEDSecurityException {
    ...
    try {
      ...
      try {
        ...
        providerName =
            XML_SIGNATURE_PROVIDER_VALUE_1.equalsIgnoreCase(providerName) ? XML_SIGNATURE_PROVIDER_VALUE_2
      ...
      throw new SEDSecurityException(InitializeException, ex, ex.getMessage());
    }
    return fac;
  }
  ...
  protected void logError(final String strMethod, String strMessage, long iStartTime, Exception ex) {
    mlgLogger.error(strMethod + ": - ERROR:" + strMethod + ":(" + (getTime() - iStartTime) + "ms)",
        ex);
  }
   ...
   * @throws SEDSecurityException
   */
  public void singDocument(KeyStore.PrivateKeyEntry certPrivateKey, Element sigParentElement,
      List<String[]> strIds, OutputStream os) throws SEDSecurityException {
    ...
    XMLStructure content =
        createXAdESQualifyingProperties(strSigId, strSigValId, strSigPropId, cert, doc);
    XMLObject xoQualifyingProperties = fac.newXMLObject(singletonList(content), null, null, null);
  ...
  private void validateSignature(Node sigNode) throws MarshalException, XMLSignatureException,
    ...
    DOMValidateContext valContext =
        new DOMValidateContext(new XMLSignatureX509KeySelector(), ndVal);
  ...
  public void validateXmlDSigSignature(File fDoc) throws SEDSecurityException,
    ...
    try {
      ...
      logError("SvevSignatureUtils.validateXmlDSigSignature: FileNotFoundException",
          ex.getMessage(), getTime(), ex);
      throw new SEDSecurityException(XMLParseException, ex, ex.getMessage());
    } finally {
      if (fis != null) {
        try {
          fis.close();
  ...
  public void validateXmlDSigSignature(InputStream is) throws SEDSecurityException,
    ...
    if (nl.getLength() == 0) {
      ...
      throw new SEDSecurityException(SignatureNotFound, "No signature found");
    }
    ...
    try {
      ...
      if (out != null) {
        try {
          ...
          mlgLogger.error("Error closing file; '" + (f != null ? f.getAbsolutePath() : "null-file")
