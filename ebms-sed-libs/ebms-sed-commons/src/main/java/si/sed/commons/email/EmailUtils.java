...
public class EmailUtils {
    ...
    private static final Logger mlgLogger =
            getLogger(EmailUtils.class.getName());

    private Address[] getAddresses(String strAddrString)
        ...
        for (int i = 0; i < lstAdr.length; i++) {
            ...
            if (asciiEncoder.canEncode(adr)) {
                ...
                if (lst.length == 2) {
                    to[i] = new InternetAddress(lst[1].replaceAll(">", ""),
                            lst[0].trim(), "UTF-8");
                }
     ...
     * @throws IOException
     */
    public void sendMailMessage(String emailAddress, String subject, String body,
            String mailConfig)
            throws
            MessagingException, NamingException, IOException {
     ...
     * @throws IOException
     */
    public void sendMailMessage(EmailData eml, String mailConfig)
        ...
        m.setSubject(subject.length() > 160 ? subject.substring(0, 160) :
                subject);
        ...
        messageBodyPart.setHeader("Content-Type", S_MIME_TXT +
                "; charset=\"utf-8\"");
        messageBodyPart.setContent(swrBody.toString(), S_MIME_TXT +
                "; charset=\"utf-8\"");
        multipart.addBodyPart(messageBodyPart);

        for (EmailAttachmentData d : eml.getAttachments()) {
            mlgLogger.info(
                    "EmailUtils.sendMailMessage: - add attachments doc  " +
        ...
        send(m);
        mlgLogger.info(
                "EmailUtils.sendMailMessage: " + eml.toString() + " - END ( " +
