...
import static javax.xml.crypto.dsig.SignatureMethod.DSA_SHA1;
import static javax.xml.crypto.dsig.SignatureMethod.RSA_SHA1;
import javax.xml.crypto.dsig.keyinfo.*;
import static javax.xml.crypto.dsig.keyinfo.X509Data.RAW_X509_CERTIFICATE_TYPE;
import static javax.xml.crypto.dsig.keyinfo.X509Data.TYPE;
...
public class XMLSignatureX509KeySelector extends KeySelector {
  ...
  private boolean algEquals(String algURI, String algName) {
    if (algName.equalsIgnoreCase("DSA") && algURI.equalsIgnoreCase(DSA_SHA1)) {
      return true;
    } else {
      return algName.equalsIgnoreCase("RSA") && algURI.equalsIgnoreCase(RSA_SHA1);
    }
  }

  /**
   * Searches the specified keystore for a certificate that matches the criteria specified in the
   * CertSelector.
   *
  ...
  private KeySelectorResult certSelect(X509Certificate xcert, SignatureMethod sm)
    ...
    /*
     * String alias = ks.getCertificateAlias(xcert); if (alias != null) { PublicKey pk =
     * ks.getCertificate(alias).getPublicKey(); // make sure algorithm is compatible with method if
     * (algEquals(sm.getAlgorithm(), pk.getAlgorithm())) { return new SimpleKeySelectorResult(pk); }
     * } return null;
     */
  }
  ...
  private String getPKAlgorithmOID(String algURI) {
    if (algURI.equalsIgnoreCase(DSA_SHA1)) {
      return "1.2.840.10040.4.1";
    } else if (algURI.equalsIgnoreCase(RSA_SHA1)) {
  ...
  /**
   * Searches the specified keystore for a certificate that matches an entry of the specified
   * X509Data and contains a public key that is compatible with the specified SignatureMethod.
   *
   * @return a KeySelectorResult containing the cert's public key if there is a match; otherwise
   *         null
   */
  private KeySelectorResult x509DataSelect(X509Data xd, SignatureMethod sm)
    ...
    while (xi.hasNext()) {
      ...
      if (o instanceof X509Certificate) {
         ...
         * else if (o instanceof X509IssuerSerial) { X509IssuerSerial xis = (X509IssuerSerial) o;
         * X509CertSelector xcs = new X509CertSelector(); try {
         * xcs.setSubjectPublicKeyAlgID(algOID); xcs.setSerialNumber(xis.getSerialNumber());
         * xcs.setIssuer(new X500Principal (xis.getIssuerName()).getName()); } catch (IOException
         * ioe) { throw new KeySelectorException(ioe); } ksr = keyStoreSelect(xcs); // check
         * X509SubjectName } else if (o instanceof String) { String sn = (String) o;
         * X509CertSelector xcs = new X509CertSelector(); try {
         * xcs.setSubjectPublicKeyAlgID(algOID); xcs.setSubject(new X500Principal(sn).getName()); }
         * catch (IOException ioe) { throw new KeySelectorException(ioe); } ksr =
         * keyStoreSelect(xcs); // check X509SKI } else if (o instanceof byte[]) { byte[] ski =
         * (byte[]) o; X509CertSelector xcs = new X509CertSelector(); try {
         * xcs.setSubjectPublicKeyAlgID(algOID); } catch (IOException ioe) { throw new
         * KeySelectorException(ioe); } // DER-encode ski - required by X509CertSelector byte[]
         * encodedSki = new byte[ski.length+2]; encodedSki[0] = 0x04; // OCTET STRING tag value
         * encodedSki[1] = (byte) ski.length; // length System.arraycopy(ski, 0, encodedSki, 2,
         * ski.length); xcs.setSubjectKeyIdentifier(encodedSki); ksr = keyStoreSelect(xcs); // check
         * X509CRL // not supported: should use CertPath API }
         */else {
